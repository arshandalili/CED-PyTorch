...
base_path = '' # base path
checkpint_path = '' # path to save checkpoints
...
class CEDCNN(pl.LightningModule):
    ...
    def loss(self, labels, res_softmax):
        ...
        for i in range(batch_size):
            if betas[i] != 1:
                ...
                #print(f"o_diff batch_loss: {batch_loss}")
                #if not (torch.isinf(batch_loss) or torch.isnan(batch_loss)):
    ...
    def step(self, batch, mode='train'):
        ...
        acc = (tp.item() + tn.item()) / \
            (tn.item() + fn.item() + tp.item() + fp.item())
...
checkpint_path += f"/{args.input}"


model = CEDCNN(lambd0, lambd1)
train_data = [json.loads(d) for d in open(base_path + 'ced-inputs/' + args.input + f'/train_ced_{args.input}.txt', "rt").readlines()]
val_data = [json.loads(d) for d in open(base_path + 'ced-inputs/' + args.input + f'/validation_ced_{args.input}.txt', "rt").readlines()]
test_data = [json.loads(d) for d in open(base_path + 'ced-inputs/' + args.input+ f'/test_ced_{args.input}.txt', "rt").readlines()]
train_dataset = SeqDataset(train_data)
val_dataset = SeqDataset(val_data)
test_dataset = SeqDataset(test_data)

logger = loggers.CSVLogger(checkpint_path)
checkpoint_callback = ModelCheckpoint(dirpath=checkpint_path, save_top_k=2, monitor="val_loss")
...
trainer = pl.Trainer(accelerator="gpu", devices=1, max_epochs=epoch_num, logger=logger, callbacks=[checkpoint_callback], log_every_n_steps=1)
